...
router.post('/', function (req, res) {
  ...
  parser.parseString(req.body.data, function (err, result) {
    ...
    fs.writeFile(path.join(__dirname, '/../uploads/', type, '/', newFileName), JSON.stringify(data), function (err) {
      if (err) {
...
function deleteVplan (type, req, res) {
  ...
  DB[type].findOne({
    ...
    fs.unlink(path.join(__dirname, '/../uploads/', type, '/', doc.filename), function (err) {
      if (err) {
...
function updateVplan (type, req, res) {
  ...
  DB[type].findOne({
    ...
    fs.unlink(path.join(__dirname, '/../uploads/', type, '/', doc.filename), function (err) {
      ...
      fs.writeFile(path.join(__dirname, '/../uploads/', type, '/', newFileName), JSON.stringify(req.body.data), function (err) {
        if (err) {
          Fehler(res, err, 'Fehler beim Schreiben der Datei')
          return
        }
...
function getVplan (type, req, res) {
  ...
  DB[type].findOne({
    ...
    fs.readFile(path.join(__dirname, '/../uploads/', type, '/', doc.filename), function (err, data) {
      if (err) {
